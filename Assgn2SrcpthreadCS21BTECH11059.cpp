...
int main()
{
    ...
    fscanf(file, "%d %d", &k, &n);
    int root = sqrt(n);
    Row = (int *)calloc(sizeof(int) , (n));
    Colum = (int *)calloc(sizeof(int) , n);
    Grid = (int *)calloc(sizeof(int) , n);
    if(k > (3 * n))
    {
    ...
    grid_it = 0;
    pthread_t threadIDs;
    // threadIDs=(pthread_t *)calloc(sizeof(pthread_t),(k+1));
    node = (struct Node *)malloc(sizeof(struct Node));
    pthread_t *tids=(pthread_t *)calloc(sizeof(pthread_t),k);
    ...
    for (int i = 0; i < k; i++)
    {
        ...
        *arg = i;
        pthread_create(&threadIDs, 0, task1, arg);
        tids[i]=threadIDs;
    }
